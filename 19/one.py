...
def inspectRecipe(recipe: Recipe):
    ...
    while len(queue) > 0:
        ...
        queue = nextMinuteQueue[0:1000] # can't quite guarantee that higher score states will lead to the best result, so we keep a couple to continue with
    sum += recipe.id * best
    print('result', recipe.id, best, recipe.id * best, sum)        
